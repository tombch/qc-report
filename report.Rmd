...
# Total samples by week, split by label 
totals_by_week = df %>%
   group_by(label, sequencing_submission_week) %>%
   summarise(n=n()) %>%
   pivot_wider(names_from=c('label'), values_from=c('n')) %>%
   replace_na(list(`N. Ireland`=0, `Pillar 1`=0, `Pillar 2`=0, `Scotland`=0, `Unknown`=0, Wales=0)) %>%
   mutate(total=Commercial+`N. Ireland`+`Pillar 1`+`Pillar 2`+Scotland+Unknown+Wales) %>%
   pivot_longer(cols=2:9) %>%
   ggplot(aes(x=sequencing_submission_week, y=value, group=name, color=name)) +
   geom_line() + 
   theme_bw()
ggsave(paste(prefix, "totals_by_week.png", sep="_"), totals_by_week, width=w, height=h)

# Total samples by week, faceted by label
totals_by_label = df %>%
...
ggsave(paste(prefix, "totals_by_label.png", sep="_"), totals_by_label, width=w, height=h)
...
totals_by_pp = df %>%
   ...
   theme_bw() +
   theme(legend.position="bottom")
ggsave(paste(prefix, "totals_by_pp.png", sep="_"), totals_by_pp, width=w, height=h)
